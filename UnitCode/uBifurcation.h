...
#ifndef uBifurcationH
#define uBifurcationH

#include "uBaseWorkShape.h"
#include "uTfeRectShape.h"
#include "uTfeRhombShape.h"
//---------------------------------------------------------------------------
class TBifurcation: public TBaseWorkShape
{
   private:
     void CreateLines();
   ...
   public:
     TBifurcation(int X, int Y, int Step, int NumberShape_Id, int Block_Id, int NumberLine_Id): TBaseWorkShape(X, Y, Step, NumberShape_Id, Block_Id, NumberLine_Id){};
     void Init();
     void Prepare();
