...
TCompositeControlFunc::TCompositeControlFunc()
{
   f_BWS = NULL;
   f_step = 0;
   f_FirstLine = NULL;
...
bool TCompositeControlFunc::ConvertWorkShape(TAlternateViewItemTFS *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2;
   TPoint POfs;
   bool res = false;
   if (AWS && (AWS->BaseWorkShape->TypeShape == 5) )
   {
...
void TCompositeControlFunc::ConvertWorkShapeFromBase(TBaseWorkShape *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2;
   TPoint POfs;
   if (AWS && (AWS->TypeShape == 5) )
   {
...
void TCompositeControlFunc::CreateLines(TBaseWorkShape *AWS)
{
   TBaseLine *Line, *Base;
...
TBaseLine* TCompositeControlFunc::GetIndexLine(int AIndex)
{
   TBaseLine* Res = NULL;
   switch (AIndex)
   {
...
void TCompositeControlFunc::Prepare()
{
   int h, d;
   int s1, s2;
   int x_offs, y_offs;
 ...
 //  f_Midl = PE1;
   ...
   dynamic_cast<TTfeEllShape*>(Shape2->BaseShape)->GetTailPoint(1, PTail);
   Line = GetIndexLine(3);
   Line->Visible = true;
   Line->ApplyCoord(0, 0, 0, 0);
  ...
  dynamic_cast<TTfeEllShape*>(Shape2->BaseShape)->GetTailPoint(0, PTail);
  Line = GetIndexLine(5);
  Line->Visible = true;
  Line->ApplyCoord(Lnb2->X1, Lnb2->Y0, PTail.x, PTail.y);


   dynamic_cast<TTfeEllShape*>(Shape2->BaseShape)->GetTailPoint(2, PTail);
...
void TCompositeControlFunc::PrepareDrawFlags()
{
   if (f_FirstRectLine)
   {
...
TPoint __fastcall TCompositeControlFunc::GetEndPoint()
{
   return f_LastLine->End;
}


TPoint __fastcall TCompositeControlFunc::GetStartPoint()
{
   return f_FirstLine->Start;
}

TBaseLine* __fastcall TCompositeControlFunc::GetEndLine()
{
   return f_LastLine;
}

TRectLine* __fastcall TCompositeControlFunc::GetFirstLine()
{
   return f_FirstRectLine;
}


void TCompositeControlFunc::MakeFirstLine(TPoint AStart, int ABend)
{
    int i;
    int m_x0;
    int m_x1;
