...
TZWork::TZWork(int X, int Y, int Step, int NumberShape_Id, int Block_Id, int NumberLine_Id): TBaseWorkShape(X, Y, Step, NumberShape_Id, Block_Id, NumberLine_Id)
{
...
//-----------------------------------------------------------------------
void TZWork::Paint(TCanvas *Canvas)
{
//    Prepare();
    ...
    if (!CompositeWorkShape)
    {
      ...
      PaintVShape(Canvas, VPoint, F_Step, F_Step, false);
...
//---------------------------------------------------------------------
int TZWork::CalcBend(int t_x1, int t_x2)
{
   int res = 0;
   if (t_x1 < t_x2) res = 2;
   if (t_x1 > t_x2) res = 3;
...
//------------------------------------------------------------------------
TPoint __fastcall TZWork::GetEndPoint()
{
   if (CompositeWorkShape)
...
//------------------------------------------------------------------------
TRectLine* __fastcall TZWork::GetFirstLine()
{
   TRectLine *L;
...
//------------------------------------------------------------------------
TRectLine* __fastcall TZWork::GetLastLine()
{
   TRectLine *L;
...
//------------------------------------------------------------------------
int __fastcall TZWork::GetTypeShape()
{
   return 2;
}
