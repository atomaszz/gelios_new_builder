...
TCompositeBifurcation::TCompositeBifurcation()
{
   f_BWS = NULL;
   f_step = 0;
   f_FirstLine = NULL;
...
bool TCompositeBifurcation::ConvertWorkShape(TAlternateViewItemTFS *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2, *mNewShape3;
   TPoint POfs;
   bool res = false;
   if (AWS && (AWS->BaseWorkShape->TypeShape == 6) )
   {
...
void TCompositeBifurcation::ConvertWorkShapeFromBase(TBaseWorkShape *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2, *mNewShape3;
   TPoint POfs;
   if (AWS && (AWS->TypeShape == 6) )
   {
...
void TCompositeBifurcation::CreateLines(TBaseWorkShape *AWS)
{
   TBaseLine *Line, *Base;
...
int TCompositeBifurcation::CalcBend(int t_x1, int t_x2)
{
   int res = 0;
   if (t_x1 < t_x2) res = 2;
   if (t_x1 > t_x2) res = 3;
...
TBaseLine* TCompositeBifurcation::GetIndexLine(int AIndex)
{
   TBaseLine* Res = NULL;
   switch (AIndex)
   {
...
void TCompositeBifurcation::Prepare()
{
   int dy1, dy2;
   ...
   R1 = Shape1->MaxRect;
   R2 = Shape2->MaxRect;
   Rhomb = dynamic_cast<TTfeRhombShape*>(GetItem(0)->BaseShape);
   Rhomb->GetTailPoint(0, RHV);
   Rhomb->GetTailPoint(1, RHV1);
   Rhomb->GetTailPoint(3, RHV2);
...
void TCompositeBifurcation::PrepareDrawFlags()
{
   if (f_FirstRectLine)
   {
...
TPoint __fastcall TCompositeBifurcation::GetEndPoint()
{
   return f_LastLine->End;
}


TPoint __fastcall TCompositeBifurcation::GetStartPoint()
{
   return f_FirstLine->Start;
}

TBaseLine* __fastcall TCompositeBifurcation::GetEndLine()
{
   return f_LastLine;
}

TRectLine* __fastcall TCompositeBifurcation::GetFirstLine()
{
   return f_FirstRectLine;
}


void TCompositeBifurcation::MakeFirstLine(TPoint AStart, int ABend)
{
    int i;
    int m_x0;
    int m_x1;
