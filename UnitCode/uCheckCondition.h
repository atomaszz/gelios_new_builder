...
#ifndef uCheckConditionH
#define uCheckConditionH
#include "uBaseWorkShape.h"
#include "uTfeRectShape.h"
#include "uTfeHexahedronShape.h"

//---------------------------------------------------------------------------
class TCheckCondition: public TBaseWorkShape
{
   private:
     void CreateLines();
   ...
   public:
     TCheckCondition(int X, int Y, int Step, int NumberShape_Id, int Block_Id, int NumberLine_Id): TBaseWorkShape(X, Y, Step, NumberShape_Id, Block_Id, NumberLine_Id){};
     void Init();
     void Prepare();
