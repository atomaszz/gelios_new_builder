...
TCompositeZWorkOR::TCompositeZWorkOR()
{
   f_BWS = NULL;
   f_step = 0;
   f_V1 = NULL;
...
bool TCompositeZWorkOR::ConvertWorkShape(TAlternateViewItemTFS *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2;
   TPoint POfs;
   bool res = false;
   if (AWS && (AWS->BaseWorkShape->TypeShape == 3) )
   {
...
void TCompositeZWorkOR::ConvertWorkShapeFromBase(TBaseWorkShape *AWS, TAlternateView* AV)
{
   TBaseShape *mNewShape1, *mNewShape2;
   TPoint POfs;
   if (AWS && (AWS->TypeShape == 3) )
   {
...
void TCompositeZWorkOR::CreateLines(TBaseWorkShape *AWS)
{
   TBaseLine *Line, *Base;
...
int TCompositeZWorkOR::CalcBend(int t_x1, int t_x2)
{
   int res = 0;
   if (t_x1 < t_x2) res = 2;
   if (t_x1 > t_x2) res = 3;
...
TBaseLine* TCompositeZWorkOR::GetIndexLine(int AIndex)
{
   TBaseLine* Res = NULL;
   switch (AIndex)
   {
...
void TCompositeZWorkOR::Prepare()
{
   int dy1, dy2;
   ...
   PrepareDrawFlags();   CreateLineVShapeComposite(VPoint, f_step, f_step, true, VPC);
...
void TCompositeZWorkOR::PrepareDrawFlags()
{
   if (f_FirstRectLine)
   {
...
TPoint __fastcall TCompositeZWorkOR::GetEndPoint()
{
   return f_LastLine->End;
}


TPoint __fastcall TCompositeZWorkOR::GetStartPoint()
{
   return f_FirstLine->Start;
}

TBaseLine* __fastcall TCompositeZWorkOR::GetEndLine()
{
   return f_LastLine;
}


TRectLine* __fastcall TCompositeZWorkOR::GetFirstLine()
{
   return f_FirstRectLine;
}


void TCompositeZWorkOR::MakeFirstLine(TPoint AStart, int ABend)
{
    int i;
    int m_x0;
    int m_x1;
